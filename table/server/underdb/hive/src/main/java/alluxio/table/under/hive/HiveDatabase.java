...
public class HiveDatabase implements UnderDatabase {
        ...
        if (mConfiguration.getBoolean(Property.ALLOW_DIFF_PART_LOC_PREFIX)) {
          for (Partition part : partitions) {
            if (part.getSd() != null && part.getSd().getLocation() != null) {
              String partUfsUri = part.getSd().getLocation();
              pathTranslator.addMapping(partUfsUri, partUfsUri);
            }
          }
        }
  ...
  @Override
  public UdbTable getTable(String tableName, Boolean ufsBypassEnabled) throws IOException {
    try {
      Table table;
      ...
      PathTranslator pathTranslator = mountAlluxioPaths(table, partitions, ufsBypassEnabled);
